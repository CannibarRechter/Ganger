...
class 'survival_desert' ( survival_base )

function survival_desert:__init()
    survival_base.__init(self,self)
end

function survival_desert:init()
	survival_base.init(self)

	LogService:Log( "survival_desert:init()" )
	
    self:PrepareSpawnPoints(32.0);
    ...
    local rulesPath = GetRulesForDifficulty( "lua/missions/survival/v2/dom_survival_desert_rules_" )

	MissionService:AddGameRule( "lua/missions/v2/dom_manager.lua", rulesPath )
	
	MissionService:AddGameRule("scripts/ganger_dom.lua", "unused")
end

return survival_desert
